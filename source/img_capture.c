...
main(int argc, char *argv[])
{
  ...
  //	}
    ...
    {
      int nm;
      struct fp_minutia **listMinutia;
      fp_img_standardize(img);
      listMinutia = fp_img_get_minutiae(img, &nm);

      if (nm > 0)
        {
          printf("numero de minucias = %d\n", nm);
          printf("height = %d\n", fp_img_get_height(img));
          printf("width = %d\n", fp_img_get_width(img));
          //          printf("img->num = %d\n",img->minutiae->num);// minutiae->num);
          struct
          {
            int num;
            struct
          ...
          for (int i = 0; i < nm; i++)
            {
              minucias.pt[i].x = listMinutia[i]->x;
              minucias.pt[i].y = listMinutia[i]->y;
            }
          FILE * pFile;
          pFile = fopen("listMinucias.int", "w");
          if (pFile != NULL)
            {
              fwrite(&minucias, 1, nm * 2 * sizeof(int) + sizeof(int), pFile);
              fclose(pFile);
            }
        }
      printf("%s\n",argv[0]);
